...
namespace ZeroMQ
{
	...
	/// </summary>
	public sealed class UFrame : Stream, ICloneable
	{
        ...
        public static UFrame FromStream(Stream stream, long i, int length)
		{
			stream.Position = i;
			if (length == 0)
                return new UFrame();

			var frame = UFrame.Create(length);
			var buf = new byte[65535];
			int bufLen, remaining, current = -1;
			do
            {
				++current;
				remaining = Math.Min(Math.Max(0, length - current * buf.Length), buf.Length);
				if (remaining < 1)
                    break;
		...
		public static UFrame CopyFrom(UFrame frame)
		{
			return frame.Duplicate();
		}

		public static UFrame Create(int size) 
		{
			if (size < 0)
			{
				throw new ArgumentOutOfRangeException("size");
			}
			return new UFrame(CreateNative(size), size);
		}

		public static UFrame CreateEmpty()
		{
			return new UFrame(CreateEmptyNative(), -1);
		}

		public UFrame(byte[] buffer)
			: this (CreateNative(buffer.Length), buffer.Length)
		{
			this.Write(buffer, 0, buffer.Length);
		}

		public UFrame(byte[] buffer, int offset, int count)
			: this(CreateNative(count), count)
		{
			this.Write(buffer, offset, count);
		}

		public UFrame(byte value)
			: this(CreateNative(1), 1)
		{ 
			this.Write(value);
		}

		public UFrame(Int16 value)
			: this(CreateNative(2), 2)
		{ 
			this.Write(value);
		}

		public UFrame(UInt16 value)
			: this(CreateNative(2), 2)
		{ 
			this.Write(value);
		}

		public UFrame(Char value)
			: this(CreateNative(2), 2)
		{ 
			this.Write(value);
		}

		public UFrame(Int32 value)
			: this(CreateNative(4), 4)
		{ 
			this.Write(value);
		}

		public UFrame(UInt32 value)
			: this(CreateNative(4), 4)
		{ 
			this.Write(value);
		}

		public UFrame(Int64 value)
			: this(CreateNative(8), 8)
		{ 
			this.Write(value);
		}

		public UFrame(UInt64 value)
			: this(CreateNative(8), 8)
		{ 
			this.Write(value);
		}
		...
		public UFrame(string str, Encoding encoding)
		{
		...
		/* protected UFrame(IntPtr data, int size)
			: this(Alloc(data, size), size)
		{ } */

		internal UFrame(DispoIntPtr frameIntPtr, int size)
			: base()
		{
		...
		public void CopyZeroTo(UFrame other)
		{
		...
		public void MoveZeroTo(UFrame other)
		{
			// zmq.msg_copy(dest, src)
			ZError error;
		...
		public UFrame Duplicate()
		{
			var frame = UFrame.CreateEmpty();
